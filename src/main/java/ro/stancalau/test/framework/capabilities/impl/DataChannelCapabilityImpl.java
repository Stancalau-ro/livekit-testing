...
public class DataChannelCapabilityImpl implements DataChannelCapability {
    ...
    public boolean sendDataMessage(String message, boolean reliable) {
        return jsExecutor.execute("sendDataMessage", Boolean.class, message, reliable, null).orElse(false);
    }
    ...
    public boolean sendDataMessageTo(String message, String recipientIdentity, boolean reliable) {
        ...
        return jsExecutor.execute("sendDataMessage", Boolean.class, message, reliable, recipients).orElse(false);
    }

    @Override
    public boolean sendDataMessageOfSize(int sizeBytes, boolean reliable) {
        return jsExecutor.execute("sendDataMessageOfSize", Boolean.class, sizeBytes, reliable).orElse(false);
    }

    @Override
    public boolean sendTimestampedDataMessage(String message, boolean reliable) {
        return jsExecutor.execute("sendTimestampedDataMessage", Boolean.class, message, reliable).orElse(false);
    }

    @Override
    public boolean hasReceivedDataMessage(String expectedContent, String fromIdentity) {
        return jsExecutor.execute("hasReceivedDataMessage", Boolean.class, expectedContent, fromIdentity).orElse(false);
    }
    ...
    public List<Map<String, Object>> getReceivedDataMessages() {
        return jsExecutor.execute("getReceivedDataMessages", List.class).orElse(Collections.emptyList());
    }
    ...
    public Map<String, Object> getDataChannelLatencyStats() {
        return jsExecutor.execute("getDataChannelLatencyStats", Map.class).orElse(Collections.emptyMap());
    }
