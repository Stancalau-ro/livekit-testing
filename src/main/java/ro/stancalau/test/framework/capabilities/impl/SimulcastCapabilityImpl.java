...
public class SimulcastCapabilityImpl implements SimulcastCapability {
    ...
    public boolean setVideoSubscribed(String publisherIdentity, boolean subscribed) {
        return jsExecutor.execute("setVideoSubscribed", Boolean.class, publisherIdentity, subscribed).orElse(false);
    }

    @Override
    public Long getRemoteVideoTrackWidthByPublisher(String publisherIdentity) {
        return jsExecutor.execute("getRemoteVideoTrackWidthByPublisher", Long.class, publisherIdentity).orElse(0L);
    }

    @Override
    public long measureVideoBitrateOverInterval(int milliseconds) {
        return jsExecutor.executeAsync("measureVideoBitrateOverInterval", Long.class, milliseconds).orElse(0L);
    }
