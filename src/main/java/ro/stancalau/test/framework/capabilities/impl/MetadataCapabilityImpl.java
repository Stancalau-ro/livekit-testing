...
public class MetadataCapabilityImpl implements MetadataCapability {
    ...
    public String getParticipantMetadata(String identity) {
        return jsExecutor.execute("getParticipantMetadata", String.class, identity).orElse("");
    }
    ...
    public boolean waitForRoomMetadataEvent(String expectedValue, int timeoutSeconds) {
        return jsExecutor.execute("waitForRoomMetadataEvent", Boolean.class, expectedValue, timeoutSeconds).orElse(false);
    }

    @Override
    public boolean waitForParticipantMetadataEvent(String identity, String expectedValue, int timeoutSeconds) {
        return jsExecutor.execute("waitForParticipantMetadataEvent", Boolean.class, identity, expectedValue, timeoutSeconds).orElse(false);
    }

    @Override
    public int getRoomMetadataEventCount() {
        return jsExecutor.execute("getRoomMetadataEventCount", Long.class).orElse(0L).intValue();
    }

    @Override
    public int getParticipantMetadataEventCount() {
        return jsExecutor.execute("getParticipantMetadataEventCount", Long.class).orElse(0L).intValue();
    }
