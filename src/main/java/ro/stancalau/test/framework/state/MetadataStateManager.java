...
public class MetadataStateManager {
  ...
  @Getter private LivekitModels.Room lastRetrievedRoomInfo;
  ...
  public void setRoomMetadata(String serviceName, String roomName, String metadata) {
    log.info(
        "Setting room metadata for room {} in service {}: {}",
        roomName,
        serviceName,
    ...
    if (!response.isSuccessful()) {
      ...
      throw new RuntimeException(
          "Failed to update room metadata: " + response.code() + " - " + errorBody);
    }
    LivekitModels.Room updatedRoom = response.body();
    if (updatedRoom != null) {
      log.info(
          "Room metadata updated successfully. New metadata: {}",
          truncateForLog(updatedRoom.getMetadata()));
    } else {
  ...
  public void updateParticipantMetadata(
    ...
    if (!response.isSuccessful()) {
      String errorBody = response.errorBody() != null ? response.errorBody().string() : "unknown";
      log.error(
          "Failed to update participant metadata. Code: {}, Error: {}", response.code(), errorBody);
      throw new RuntimeException(
          "Failed to update participant metadata: " + response.code() + " - " + errorBody);
    }
  ...
  public boolean waitForRoomMetadataEvent(String participantName, String expectedValue) {
    LiveKitMeet meetInstance = meetSessionStateManager.getMeetInstance(participantName);
    log.info(
        "Waiting for room metadata event for {} with value '{}'", participantName, expectedValue);
    boolean found =
  ...
  public boolean waitForParticipantMetadataEvent(
        ...
        BrowserPollingHelper.pollForCondition(
            () -> {
              ...
              return events.stream()
                  .anyMatch(
                      e ->
                          targetIdentity.equals(String.valueOf(e.get("participantIdentity")))
    ...
    if (found) {
      ...
      List<Map<String, Object>> finalEvents =
          meetInstance.getMetadata().getParticipantMetadataEvents();
      log.warn(
  ...
  public boolean pollUntilRoomMetadataEquals(
      String serviceName, String roomName, String expectedMetadata) {
