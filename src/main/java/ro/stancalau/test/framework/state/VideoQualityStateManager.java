...
public class VideoQualityStateManager {
  ...
  public Long pollForLowerQualityWidth(String subscriber, String publisher, int maxWidth) {
    LiveKitMeet meetInstance = meetSessionStateManager.getMeetInstance(subscriber);
    Long width =
        pollForLowerQualityLayer(
            meetInstance, publisher, maxWidth, LOWER_LAYER_TIMEOUT_MS, POLL_DELAY_MS);
    return width != null ? width : 0L;
  }
  ...
  public boolean waitForTrackStreamState(
      String subscriber, String publisher, String expectedState, int timeoutMs) {
    LiveKitMeet meetInstance = meetSessionStateManager.getMeetInstance(subscriber);
  ...
  public int measureCurrentBitrate(String participantName) {
    LiveKitMeet meetInstance = meetSessionStateManager.getMeetInstance(participantName);
    long currentBitrateKbps =
        meetInstance.getSimulcast().measureVideoBitrateOverInterval(BITRATE_MEASUREMENT_MS);
  ...
  private Long pollForVideoWidth(
      LiveKitMeet meetInstance, String publisherIdentity, long timeoutMs, long delayMs) {
