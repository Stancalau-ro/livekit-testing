...
public class EgressContainer extends GenericContainer<EgressContainer> {
    ...
    public static EgressContainer createContainer(
        ...
        EgressContainer container =
                new EgressContainer(egressImage, network, apiKey, apiSecret, livekitWsUrl).withExposedPorts(GRPC_PORT);
    ...
    public static EgressContainer createContainerWithS3(
        ...
        container = container
                ...
                .waitingFor(Wait.forLogMessage(".*service ready.*", 1).withStartupTimeout(Duration.ofSeconds(60)));
    ...
    private static EgressContainer configureOutputDirectories(
        ...
        container =
                bindOutputDirectory(container, alias, logDestinationPath, "video-recordings", "/out/video-recordings");
    ...
    private static EgressContainer bindOutputDirectory(
            EgressContainer container, String alias, String logDestinationPath, String dirType, String containerPath) {
    ...
    private static EgressContainer configureContainerCapabilities(EgressContainer container) {
        return container.withCreateContainerCmdModifier(cmd ->
                cmd.getHostConfig().withCapAdd(Capability.SYS_ADMIN).withSecurityOpts(List.of("apparmor:unconfined")));
    }
    ...
    private static EgressContainer configureEgressConfig(
        ...
        if (redisUrl != null) {
            ...
            if (configFile.exists()) {
                container = container
                        .withFileSystemBind(configFile.getAbsolutePath(), "/egress.yaml", BindMode.READ_ONLY)
...
""",
    ...
    private static String createEgressConfigBodyWithS3(
            String apiKey, String apiSecret, String wsUrl, String redisUrl, String alias, S3Config s3Config) {
        return String.format(
                """
            api_key: %s
            api_secret: %s
