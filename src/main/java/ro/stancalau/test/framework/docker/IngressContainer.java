...
public class IngressContainer extends GenericContainer<IngressContainer> {
  ...
  public static IngressContainer createContainer(
    ...
    IngressContainer container =
        new IngressContainer(ingressImage, network)
  ...
  private static IngressContainer configureIngressConfig(
      IngressContainer container, String apiKey, String apiSecret, String wsUrl, String redisUrl) {
  ...
  private static IngressContainer configureNetworkAndWaiting(
      IngressContainer container, Network network, String alias) {
    return container
        .withNetwork(network)
        .withNetworkAliases(alias)
        .waitingFor(
            Wait.forHttp("/").forPort(HEALTH_PORT).withStartupTimeout(Duration.ofSeconds(120)));
  }

  private static String createIngressConfigBody(
      String apiKey, String apiSecret, String wsUrl, String redisUrl) {
    return String.format(
        """
            ...
            """,
        apiKey, apiSecret, wsUrl, redisUrl, RTMP_PORT, WHIP_PORT, HEALTH_PORT);
  }
