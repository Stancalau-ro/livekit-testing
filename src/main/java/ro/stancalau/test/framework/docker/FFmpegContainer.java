...
public class FFmpegContainer extends GenericContainer<FFmpegContainer> {
    ...
    public static FFmpegContainer createRtmpStream(
        ...
        container =
                container.withNetwork(network).withNetworkAliases(alias).withCommand(command.toArray(new String[0]));

        log.info(
                "Created FFmpeg container {} streaming to {} for {} seconds at {} {}fps",
                alias,
                fullRtmpUrl,
                durationSeconds,
                resolution,
    ...
    public static FFmpegContainer createRtmpStreamWithWrongKey(
            String alias, Network network, String rtmpUrl, int durationSeconds, @Nullable String logDestinationPath) {
        return createRtmpStream(alias, network, rtmpUrl, "invalid-stream-key", durationSeconds, logDestinationPath);
    }

    private static List<String> buildRtmpCommand(
            String rtmpUrl, String resolution, int framerate, int durationSeconds) {
