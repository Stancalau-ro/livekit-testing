...
public class LiveKitContainer extends GenericContainer<LiveKitContainer> {
  ...
  public static LiveKitContainer createContainer(
    ...
    LiveKitContainer container =
        new LiveKitContainer(liveKitImage, network)
            .withExposedPorts(HTTP_PORT)
            .waitingFor(
                Wait.forLogMessage(".*starting LiveKit server.*", 1)
    ...
    if (configFilePath != null && !configFilePath.isEmpty()) {
      ...
      if (configFile.exists()) {
        container =
            container
                .withFileSystemBind(
                    configFile.getAbsolutePath(), "/config.yaml", BindMode.READ_ONLY)
    ...
    outputDir.mkdirs();
    container =
        container.withFileSystemBind(outputDir.getAbsolutePath(), "/out", BindMode.READ_WRITE);
  ...
  public static LiveKitContainer createContainer(
      Network network, String configFilePath, String livekitVersion) {
