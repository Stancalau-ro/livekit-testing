...
public class TestConfig {
  ...
  private static final String DEFAULT_RECORDING_MODE = "failed";
  ...
  /**
   * Gets the LiveKit version to use for tests. Priority: System Property > Environment Variable >
   * Gradle Property > Default
   *
   * <p>Usage: - System Property: -Dlivekit.version=v1.8.5 - Environment Variable:
   * LIVEKIT_VERSION=v1.8.5 - Gradle Property: -Plivekit_docker_version=v1.8.5 - Default: v1.8.4
   */
  ...
  /**
   * Gets the egress version to use for tests. Priority: System Property > Environment Variable >
   * Gradle Property > Default
   *
   * <p>Usage: - System Property: -Degress.version=v1.8.5 - Environment Variable:
   * EGRESS_VERSION=v1.8.5 - Gradle Property: -Pegress_docker_version=v1.8.5 - Default: v1.8.4
   */
  ...
  /**
   * Gets the VNC recording mode for browser tests. Priority: System Property > Environment Variable
   * > Default
   *
   * <p>Valid values: - "skip": No recordings - "all": Record all tests (passed and failed) -
   * "failed": Record only failed tests
   *
   * <p>Usage: - System Property: -Drecording.mode=failed - Environment Variable:
   * RECORDING_MODE=skip - Default: failed
   */
  ...
  private static String validateAndNormalizeRecordingMode(String mode) {
    ...
    log.warn(
        "Invalid recording mode '{}'. Valid options are: {}. Falling back to default: '{}'",
        mode,
  ...
  /**
   * Resolves the configuration path for a given profile using the current LiveKit version. Falls
   * back to the latest available version if the current version is not found.
   *
  ...
  public static String resolveConfigPath(String profileName) {
    ...
    if (versionedConfig.exists()) {
      log.debug(
          "Using configuration for version {} and profile {}: {}",
          currentVersion,
          profileName,
          versionedPath);
    ...
    log.warn(
        "Configuration not found for version {} and profile {}. Searching for latest available version...",
        currentVersion,
        profileName);
  ...
  private static String findLatestVersionConfig(String profileName) {
    ...
    for (File versionDir : versionDirs) {
      String candidatePath =
          PathUtils.livekitConfigPath(versionDir.getName(), profileName, "config.yaml");
      File candidateConfig = new File(candidatePath);

      if (candidateConfig.exists()) {
        log.warn(
            "Using fallback configuration for profile {} from version {}: {}",
            profileName,
            versionDir.getName(),
            candidatePath);
