...
public class ImageValidationUtils {
    ...
    public static boolean isValidImage(File imageFile) {
        ...
        try {
            ...
            if (image.getWidth() <= 0 || image.getHeight() <= 0) {
                log.error("Image has invalid dimensions: {}x{} for file: {}", 
                         image.getWidth(), image.getHeight(), imageFile.getAbsolutePath());
                return false;
            }
            
            log.debug("Valid image found: {} ({}x{}, {} bytes)", 
    ...
    public static boolean hasMinimumSize(File imageFile, long minimumBytes) {
        ...
        if (!hasMinSize) {
            log.warn("Image file {} is smaller than minimum size: {} bytes (minimum: {} bytes)", 
                    imageFile.getName(), fileSize, minimumBytes);
        }
    ...
    public static boolean hasValidDimensions(File imageFile, int minWidth, int minHeight) {
        ...
        try {
            ...
            if (!validWidth || !validHeight) {
                log.warn("Image {} dimensions {}x{} are smaller than minimum {}x{}", 
                        imageFile.getName(), image.getWidth(), image.getHeight(), minWidth, minHeight);
            }
