...
public class BrowserPollingHelper {
  ...
  public <T> T pollUntil(
      Supplier<T> supplier, Predicate<T> condition, long timeoutMs, long delayMs) {
  ...
  public <T> T pollUntilOrThrow(
      Supplier<T> supplier, Predicate<T> condition, String operation, String context) {
    return pollUntilOrThrow(
        supplier, condition, DEFAULT_TIMEOUT_MS, DEFAULT_DELAY_MS, operation, context);
  }
  ...
  public void pollForConditionOrThrow(
      Supplier<Boolean> condition, String operation, String context) {
