...
class FileUtilsTest {
    ...
    void sanitizeFileName_realWorldExamples_workCorrectly() {
        assertEquals("Record_video_from_publisher", 
                FileUtils.sanitizeFileName("Record video from publisher"));
        assertEquals("LiveKit_WebRTC_Publish", 
                FileUtils.sanitizeFileName("LiveKit WebRTC Publish"));
        assertEquals("Test_with_special_chars_123", 
                FileUtils.sanitizeFileName("Test with @special #chars! 123"));
        assertEquals("", 
                FileUtils.sanitizeFileName("@#$%^&*()"));
        assertEquals("multiple_spaces_normalized", 
                FileUtils.sanitizeFileName("   multiple    spaces     normalized   "));
    }
    ...
    void sanitizePath_withSpacesInSegments_sanitizesSegments() {
        assertEquals("path_with_spaces/to/file_name", 
                FileUtils.sanitizePath("path with spaces/to/file name"));
        assertEquals("multiple_segments/with_spaces/here", 
                FileUtils.sanitizePath("multiple segments/with spaces/here"));
    }

    @Test
    void sanitizePath_withInvalidCharsInSegments_sanitizesSegments() {
        assertEquals("path_to/file_name", 
                FileUtils.sanitizePath("path@to/file#name"));
        assertEquals("test_folder/sub_dir/file", 
                FileUtils.sanitizePath("test@folder/sub#dir/file"));
    }

    @Test
    void sanitizePath_withBackslashes_convertsToForwardSlashes() {
        assertEquals("windows/style/path", 
                FileUtils.sanitizePath("windows\\style\\path"));
        assertEquals("mixed/slash/path", 
                FileUtils.sanitizePath("mixed\\slash/path"));
    }

    @Test
    void sanitizePath_realWorldExamples_workCorrectly() {
        assertEquals("out/bdd/scenarios/LiveKit_Test/Test_Scenario", 
                FileUtils.sanitizePath("out/bdd/scenarios/LiveKit Test/Test Scenario"));
        assertEquals("feature/name/with_special_chars", 
                FileUtils.sanitizePath("feature\\name\\with@special#chars"));
        assertEquals("deep/nested/path/with_multiple_issues", 
