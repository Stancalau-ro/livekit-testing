...
public class LiveKitWebhookSteps {
  ...
  public void aMockHttpServerIsRunningInAContainerWithServiceName(String serviceName) {
    if (!ManagerProvider.containers().isContainerRunning(serviceName)) {
      ...
      MockHttpServerContainer mockServer =
          new MockHttpServerContainer(currentScenarioLogPath, serviceName);
      ...
      log.info(
          "Mock HTTP server started successfully at: {}", mockServer.getNetworkUrl(serviceName));
  ...
  public void theMockServerShouldHaveReceivedARequestContaining(
      String serviceName, String expectedContent) {
    MockHttpServerContainer mockServer =
    ...
    try {
      ...
      if (foundEvent.isEmpty()) {
        ...
        fail(
            "Expected mock server to have received webhook event of type '"
                + expectedContent
                + "' but received events: "
  ...
  public void theMockServerShouldHaveReceivedAnEventForRoom(
    ...
    try {
      ...
      if (foundEvent.isEmpty()) {
        List<WebhookEvent> allEvents = webhookService.getWebhookEvents(mockServerClient);
        fail(
            "Expected mock server to have received '"
                + eventType
                + "' event for room '"
                + roomName
                + "' but received events: "
  ...
  public void theMockServerShouldHaveReceivedMultipleEventsForRoom(
    ...
    try {
      ...
      if (actualCount != expectedCount) {
        List<String> allEventsDescription =
            webhookEvents.stream()
                .map(
                    e ->
                        e.getEvent()
                            + (e.getRoom() != null ? " (room: " + e.getRoom().getName() + ")" : ""))
                .toList();
  ...
  public void theMockServerShouldHaveReceivedAnEventForParticipant(
    ...
    try {
      MockServerClient mockServerClient = mockServer.getMockServerClient();
      Optional<WebhookEvent> foundEvent =
          webhookEventPoller.waitForEventByTypeParticipantAndRoom(
                        ...
                        e ->
                            e.getEvent()
                                + (e.getParticipant() != null
                                    ? " (participant: " + e.getParticipant().getIdentity() + ")"
                                    : "")
                                + (e.getRoom() != null
      ...
      log.info(
          "Found {} event for participant {} in room {}", eventType, participantIdentity, roomName);
  ...
  public void theMockServerShouldHaveReceivedAnEventForTrackType(
    ...
    try {
      MockServerClient mockServerClient = mockServer.getMockServerClient();
      Optional<WebhookEvent> foundEvent =
          webhookEventPoller.waitForEventByTypeTrackTypeAndRoom(
                ...
                + allEvents.stream()
                    .map(
                        e ->
                            e.getEvent()
                                + (e.getTrack() != null
                                    ? " (track type: " + e.getTrack().getEffectiveType() + ")"
                                    : "")
                                + (e.getRoom() != null
                                    ? " (room: " + e.getRoom().getName() + ")"
                                    : ""))
  ...
  public void theMockServerShouldHaveReceivedAnEventForTrackSource(
    ...
    try {
      MockServerClient mockServerClient = mockServer.getMockServerClient();
      Optional<WebhookEvent> foundEvent =
          webhookEventPoller.waitForEventByTypeTrackSourceAndRoom(
                ...
                + allEvents.stream()
                    .map(
                        e ->
                            e.getEvent()
                                + (e.getTrack() != null
                                    ? " (track source: " + e.getTrack().getSource() + ")"
                                    : "")
                                + (e.getRoom() != null
                                    ? " (room: " + e.getRoom().getName() + ")"
                                    : ""))
  ...
  @Then(
      "{string} should have received a {string} event for {string} track from {string} in room {string}")
  public void theMockServerShouldHaveReceivedAnEventForTrackTypeAndSource(
    ...
    try {
      MockServerClient mockServerClient = mockServer.getMockServerClient();
      Optional<WebhookEvent> foundEvent =
          webhookEventPoller.waitForEventByTypeTrackTypeSourceAndRoom(
      ...
      log.info(
          "Found {} event for {} track from {} in room {}",
          eventType,
          trackType,
          trackSource,
          roomName);
  ...
  public void theMockServerShouldHaveReceivedExactlyWebhookEvents(
      String serviceName, int expectedCount) {
    MockHttpServerContainer mockServer =
        ManagerProvider.containers().getContainer(serviceName, MockHttpServerContainer.class);
    ...
    try {
      ...
      Optional<WebhookEvent> foundEvent =
          webhookEvents.stream()
              .filter(event -> eventType.equals(event.getEvent()))
              .filter(
                  event -> event.getRoom() != null && roomName.equals(event.getRoom().getName()))
              .findFirst();

      assertFalse(
          foundEvent.isPresent(),
          String.format(
              "Expected NOT to find '%s' event for room '%s' but it was present",
              eventType, roomName));
  ...
  @Then(
      "{string} should not have received a {string} event for participant {string} in room {string}")
  ...
  public void theMockServerShouldHaveReceivedAnEventForParticipantWithMetadata(
    ...
    WebhookEvent event =
        waitForParticipantEvent(serviceName, eventType, participantIdentity, roomName);
  ...
  public void theMockServerShouldHaveReceivedAnEventForRoomWithMetadata(
    ...
    MockServerClient client = getMockServerClient(serviceName);
    Optional<WebhookEvent> foundEvent =
        webhookEventPoller.waitForEventByTypeAndRoom(client, eventType, roomName);
    if (foundEvent.isEmpty()) {
      List<WebhookEvent> allEvents = webhookService.getWebhookEvents(client);
      fail(
          "Expected mock server to have received '"
