...
public class VideoQualitySteps {
  ...
  public void shouldBeReceivingLowQualityVideoFrom(String subscriber, String publisher) {
    Long receivedWidth =
        ManagerProvider.videoQuality().getReceivedVideoWidth(subscriber, publisher);
    assertTrue(
  ...
  public void shouldBeReceivingLowerQualityLayerFrom(String subscriber, String publisher) {
    Long receivedWidth =
        ManagerProvider.videoQuality()
            .pollForLowerQualityWidth(subscriber, publisher, LOW_QUALITY_MAX_WIDTH);
    assertTrue(
        receivedWidth > 0 && receivedWidth <= LOW_QUALITY_MAX_WIDTH,
        subscriber
  ...
  public void videoTrackShouldBePausedFor(String publisher, String subscriber) {
    boolean isUnsubscribed =
        ManagerProvider.videoQuality()
            .waitForTrackStreamState(subscriber, publisher, "unsubscribed", 15000);
    assertTrue(
        isUnsubscribed,
        publisher
  ...
  public void videoTrackShouldBeActiveFor(String publisher, String subscriber) {
    boolean isActive =
        ManagerProvider.videoQuality()
            .waitForTrackStreamState(subscriber, publisher, "active", 10000);
    if (!isActive) {
      boolean isPending =
          ManagerProvider.videoQuality()
              .waitForTrackStreamState(subscriber, publisher, "pending", 2000);
  ...
  public void videoPublishBitrateShouldHaveDropped(String participantName, int minDropPercent) {
    ...
    assertTrue(
        baselineBitrate > 0, "Baseline bitrate should have been measured for " + participantName);

    int currentBitrate = ManagerProvider.videoQuality().measureCurrentBitrate(participantName);
    int actualDropPercent =
