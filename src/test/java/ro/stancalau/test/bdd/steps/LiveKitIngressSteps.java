...
public class LiveKitIngressSteps {
    ...
    public void ingressServiceRunning(String serviceName, String livekitServiceName, String redisServiceName) {
        ContainerStateManager containerManager = ManagerProvider.containers();
        LiveKitContainer livekit = containerManager.getContainer(livekitServiceName, LiveKitContainer.class);
        ...
        ingress.start();
        assertTrue(ingress.isRunning(), "Ingress container with service name " + serviceName + " should be running");
    ...
    public void createRtmpIngress(String identity, String roomName, String serviceName) throws Exception {
    ...
    public void createRtmpIngressWithDisplayName(
        ...
        IngressInfo info = client.createIngress(
                        identity,
                        roomName,
                        identity,
                        displayName,
    ...
    public void createRtmpIngressWithPreset(String identity, String roomName, String serviceName, String preset)
            throws Exception {
        IngressServiceClient client = getIngressClient(serviceName);
    ...
    public void createUrlIngress(String identity, String roomName, String serviceName, String url) throws Exception {
        IngressServiceClient client = getIngressClient(serviceName);

        IngressInfo info = client.createIngress(
                        identity, roomName, identity, identity, IngressInput.URL_INPUT, null, null, null, null, url)
                .execute()
                .body();
    ...
    public void startStreamingWithDuration(String identity, String ingressServiceName, int durationSeconds) {
        IngressInfo info = ManagerProvider.ingress().getIngress(identity);
        assertNotNull(info, "Ingress for " + identity + " not found");

        ContainerStateManager containerManager = ManagerProvider.containers();
        IngressContainer ingressContainer = containerManager.getContainer(ingressServiceName, IngressContainer.class);

        String containerAlias = "ffmpeg-" + identity;
        String logPath = PathUtils.containerLogPath(getCurrentScenarioLogPath(), "docker", containerAlias);
        ...
        log.info("Started RTMP stream for {} to {} for {} seconds", identity, ingressServiceName, durationSeconds);
    }

    @When("{string} starts streaming via RTMP to {string} with resolution {string}")
    public void startStreamingWithResolution(String identity, String ingressServiceName, String resolution) {
        startStreamingWithResolutionAndDuration(
                identity, ingressServiceName, resolution, DEFAULT_STREAM_DURATION_SECONDS);
    }

    @When("{string} starts streaming via RTMP to {string} with resolution {string} for {int} seconds")
    public void startStreamingWithResolutionAndDuration(
            String identity, String ingressServiceName, String resolution, int durationSeconds) {
        IngressInfo info = ManagerProvider.ingress().getIngress(identity);
        assertNotNull(info, "Ingress for " + identity + " not found");
        ...
        log.info(
                "Started RTMP stream for {} to {} with resolution {} for {} seconds",
                identity,
                ingressServiceName,
                resolution,
                durationSeconds);
    ...
    public void startStreamingWithWrongKey(String identity, String ingressServiceName) {
        ...
        FFmpegContainer ffmpeg = FFmpegContainer.createRtmpStreamWithWrongKey(
    ...
    public void stopStreaming(String identity, String ingressServiceName) {
        ...
        if (containerAlias != null) {
            try {
                FFmpegContainer ffmpeg = containerManager.getContainer(containerAlias, FFmpegContainer.class);
    ...
    public void verifyRoomIngressCount(String roomName, String serviceName, int expectedCount) throws Exception {
        IngressServiceClient client = getIngressClient(serviceName);
        List<IngressInfo> ingresses =
                client.listIngress(roomName, null).execute().body();
        assertNotNull(ingresses, "Ingress list should not be null");
        assertEquals(
                expectedCount, ingresses.size(), "Room " + roomName + " should have " + expectedCount + " ingresses");
    }

    @Then("room {string} on {string} should have ingresses for {string}")
    public void verifyRoomIngressIdentities(String roomName, String serviceName, String expectedIdentities)
            throws Exception {
        IngressServiceClient client = getIngressClient(serviceName);
        List<IngressInfo> ingresses =
                client.listIngress(roomName, null).execute().body();
        ...
        for (String identity : identities) {
            boolean found = ingresses.stream()
                    .anyMatch(info -> info.getParticipantIdentity().equals(identity)
    ...
    public void verifyIngressRtmpUrl(String identity, String serviceName) {
        ...
        assertTrue(info.getUrl().startsWith("rtmp://"), "Ingress URL should be an RTMP URL: " + info.getUrl());
    }
    ...
    public void verifyIngressState(String identity, String serviceName, IngressState expectedState) throws Exception {
    ...
    public void verifyIngressStateWithTimeout(
            String identity, String serviceName, IngressState expectedState, int timeoutSeconds) throws Exception {
        IngressInfo storedInfo = ManagerProvider.ingress().getIngress(identity);
        assertNotNull(storedInfo, "Ingress for " + identity + " not found");
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            List<IngressInfo> ingresses = client.listIngress(null, storedInfo.getIngressId())
                    .execute()
                    .body();
    ...
    public void verifyIngressParticipantName(String identity, String serviceName, String expectedName) {
        IngressInfo info = ManagerProvider.ingress().getIngress(identity);
        assertNotNull(info, "Ingress for " + identity + " not found");
    ...
    public void verifyIngressRemainsInState(String identity, String serviceName, IngressState expectedState)
            throws Exception {
    ...
    public void ingressIsInState(String identity, String serviceName, IngressState expectedState) throws Exception {
        verifyIngressStateWithTimeout(identity, serviceName, expectedState, 30);
    }

    @Given("the ingress for {string} on {string} is {ingressState} within {int} seconds")
    public void ingressIsInStateWithinTimeout(
            String identity, String serviceName, IngressState expectedState, int timeoutSeconds) throws Exception {
        verifyIngressStateWithTimeout(identity, serviceName, expectedState, timeoutSeconds);
    }

    private IngressServiceClient getIngressClient(String serviceName) {
        LiveKitContainer livekit = ManagerProvider.containers().getContainer(serviceName, LiveKitContainer.class);
