...
public class LiveKitIngressSteps {
    ...
    public void createRtmpIngress(String ingressName, String roomName, String identity) throws Exception {
    ...
    public void createRtmpIngressWithName(String ingressName, String roomName, String identity, String displayName) throws Exception {
        ...
        log.info("Created RTMP ingress {} for room {} with URL: {} and stream key: {}",
            ingressName, roomName, info.getUrl(), info.getStreamKey());
    }

    @When("an RTMP ingress {string} is created for room {string} with identity {string} and video preset {string}")
    public void createRtmpIngressWithPreset(String ingressName, String roomName, String identity, String preset) throws Exception {
        IngressServiceClient client = getIngressClient();

        LivekitIngress.IngressVideoEncodingPreset videoPreset = LivekitIngress.IngressVideoEncodingPreset.valueOf(preset);
    ...
    public void createUrlIngress(String ingressName, String roomName, String url, String identity) throws Exception {
        IngressServiceClient client = getIngressClient();
    ...
    public void sendRtmpStreamWithDuration(String ingressName, int durationSeconds) {
        ...
        ContainerStateManager containerManager = ManagerProvider.containers();
        IngressContainer ingressContainer = containerManager.getContainer("ingress1", IngressContainer.class);
        ...
        log.info("Started RTMP stream to ingress {} at {} for {} seconds", ingressName, rtmpUrl, durationSeconds);
    }
    ...
    private void waitForIngressPublishing(String ingressName, String ingressId, int timeoutSeconds) {
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            try {
                ...
                if (ingresses != null && !ingresses.isEmpty()) {
                    ...
                    String currentState = current.getState().getStatus().name();
                    ...
                    if ("ENDPOINT_PUBLISHING".equals(currentState)) {
                        log.info("Ingress {} reached ENDPOINT_PUBLISHING after {}ms",
                            ingressName, System.currentTimeMillis() - startTime);
                        return;
                    }

                    log.debug("Ingress {} current state: {}, waiting for ENDPOINT_PUBLISHING", ingressName, currentState);
                }
        ...
        log.warn("Ingress {} did not reach ENDPOINT_PUBLISHING within {} seconds", ingressName, timeoutSeconds);
    }
    ...
    public void sendRtmpStreamWithResolution(String ingressName, String resolution) {
        ...
        log.info("Started RTMP stream to ingress {} at {} with resolution {}", ingressName, rtmpUrl, resolution);
    }
    ...
    public void verifyIngressState(String ingressName, String expectedState) throws Exception {
        ...
        IngressServiceClient client = getIngressClient();
        List<IngressInfo> ingresses = client.listIngress(null, storedInfo.getIngressId()).execute().body();
        ...
        assertEquals(expectedState, currentInfo.getState().getStatus().name(),
            "Ingress state mismatch. Expected: " + expectedState + ", Actual: " + currentInfo.getState().getStatus().name());
    }

    @Then("the ingress {string} should be in state {string} within {int} seconds")
    public void verifyIngressStateWithTimeout(String ingressName, String expectedState, int timeoutSeconds) throws Exception {
        IngressInfo storedInfo = ManagerProvider.ingress().getIngress(ingressName);
        assertNotNull(storedInfo, "Ingress " + ingressName + " not found");
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            ...
            if (ingresses != null && !ingresses.isEmpty()) {
                IngressInfo currentInfo = ingresses.get(0);
                String currentState = currentInfo.getState().getStatus().name();

                if (currentState.equals(expectedState)) {
                    log.info("Ingress {} reached state {} after {}ms",
                        ingressName, expectedState, System.currentTimeMillis() - startTime);
                    return;
                }

                log.debug("Ingress {} current state: {}, waiting for: {}", ingressName, currentState, expectedState);
            }

            TimeUnit.MILLISECONDS.sleep(STATE_POLL_INTERVAL_MS);
        }

        fail("Ingress " + ingressName + " did not reach state " + expectedState + " within " + timeoutSeconds + " seconds");
    }
    ...
    public void verifyIngressNotExist(String ingressName) throws Exception {
        ...
        if (storedInfo != null) {
            IngressServiceClient client = getIngressClient();
            List<IngressInfo> ingresses = client.listIngress(null, storedInfo.getIngressId()).execute().body();
            assertTrue(ingresses == null || ingresses.isEmpty(),
                "Ingress " + ingressName + " should not exist but was found");
        }
    }

    @Then("the ingress {string} should remain in state {string}")
    public void verifyIngressRemainsInState(String ingressName, String expectedState) throws Exception {
    ...
    public void verifyIngressListContains(String ingressName) {
        ...
        boolean found = ingresses.stream()
            .anyMatch(info -> info.getName().equals(ingressName));
    ...
    @And("the ingress {string} is in state {string}")
    public void ingressIsInState(String ingressName, String expectedState) throws Exception {
        verifyIngressStateWithTimeout(ingressName, expectedState, 30);
    }

    private IngressServiceClient getIngressClient() {
        LiveKitContainer livekit = ManagerProvider.containers().getContainer("livekit1", LiveKitContainer.class);
