...
public class VideoRecordingEgressSteps {
  ...
  public void aRedisServerIsRunningInContainer(String serviceName) {
    ...
    RedisContainer redisContainer =
        RedisContainer.createContainer(
            serviceName, containerManager.getOrCreateNetwork(), serviceLogPath);

    redisContainer.start();
    assertTrue(
        redisContainer.isRunning(),
        "Redis container with service name " + serviceName + " should be running");
  ...
  public void aMinIOServerIsRunningInContainerWithCredentials(
    ...
    MinIOContainer minioContainer =
        MinIOContainer.createContainer(
            serviceName,
    ...
    assertTrue(
        ...
        "MinIO container with service name " + serviceName + " should be running");
    containerManager.registerContainer(serviceName, minioContainer);
    log.info(
        "MinIO service {} started with endpoint: {}",
        serviceName,
  ...
  public void bucketIsCreatedInMinIO(String bucketName, String minioServiceName) {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    MinIOContainer minioContainer =
        containerManager.getContainer(minioServiceName, MinIOContainer.class);
  ...
  public void aLiveKitVideoRecordingEgressServiceIsRunningInContainer(
      String serviceName, String livekitServiceName) {
  ...
  public void aLiveKitEgressServiceIsRunningInContainer(
      String serviceName, String livekitServiceName) {
    createEgressContainer(serviceName, livekitServiceName, null);
  }
  ...
  @Given(
      "track IDs are captured for participant {string} in room {string} using LiveKit service {string}")
  public void captureTrackIds(
      String participantIdentity, String roomName, String livekitServiceName) throws Exception {
    RoomServiceClient roomClient =
        ManagerProvider.getRoomClientManager().getRoomServiceClient(livekitServiceName);
    ...
    LivekitModels.ParticipantInfo targetParticipant =
        participants.stream()
    ...
    assertNotNull(
        targetParticipant, "Participant " + participantIdentity + " not found in room " + roomName);
    ...
    for (LivekitModels.TrackInfo track : targetParticipant.getTracksList()) {
      ...
      log.info(
          "Captured track ID for participant {}: {} track = {}",
          participantIdentity,
          trackType,
          trackId);
    }
    ...
    assertTrue(
        trackIds.containsKey("audio"),
        "Audio track not found for participant " + participantIdentity);
    assertTrue(
        trackIds.containsKey("video"),
        "Video track not found for participant " + participantIdentity);
  }

  @When(
      "the system starts room composite recording for room {string} using LiveKit service {string}")
  public void startRoomCompositeRecording(String roomName, String livekitServiceName)
      throws Exception {
  ...
  public void startRoomCompositeRecordingWithS3(
      String roomName, String livekitServiceName, String bucketName) throws Exception {
  ...
  public void startRoomCompositeRecordingWithS3AndPrefix(
      String roomName, String livekitServiceName, String bucketName, String prefix)
      throws Exception {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    ...
    LivekitEgress.S3Upload s3Upload =
        createS3Upload(minioContainer, bucketName, roomName, timestamp);
  ...
  public void startTrackCompositeRecording(
      String participantIdentity, String roomName, String livekitServiceName) throws Exception {
  ...
  public void startTrackCompositeRecordingWithS3(
      ...
      throws Exception {
    startTrackCompositeRecordingWithS3AndPrefix(
        participantIdentity, roomName, livekitServiceName, bucketName, "");
  }
  ...
  public void startTrackCompositeRecordingWithS3AndPrefix(
    ...
    Map<String, String> trackIds =
        ManagerProvider.getEgressStateManager().getTrackIds(participantIdentity);
    ...
    ManagerProvider.getEgressStateManager()
        .storeActiveRecording(participantIdentity + "_track", egressId);
  ...
  @When(
      "the system stops track composite recording for participant {string} using LiveKit service {string}")
  public void stopTrackCompositeRecording(String participantIdentity, String livekitServiceName)
      throws Exception {
    String egressId =
        ManagerProvider.getEgressStateManager().getActiveRecording(participantIdentity + "_track");
    assertNotNull(
        egressId,
        "No active track composite recording found for participant " + participantIdentity);
  ...
  @When(
      "the system stops room composite recording for room {string} using LiveKit service {string}")
  public void stopRoomCompositeRecording(String roomName, String livekitServiceName)
      throws Exception {
    ...
    stopEgressRecording(
        egressId, livekitServiceName, roomName, "room composite recording for room " + roomName);
  }
  ...
  public void verifyTrackCompositeRecordingFileExists(String participantIdentity)
      throws InterruptedException {
    verifyLocalRecordingFile(
        "track-composite-" + participantIdentity, "participant " + participantIdentity);
  }

  @Then("the recording file should exist in MinIO bucket {string} for room {string}")
  public void verifyRecordingFileExistsInMinIO(String bucketName, String roomName)
      throws InterruptedException {
    verifyRecordingFileExistsInMinIOWithPrefix(bucketName, "", roomName);
  }

  @Then(
      "the recording file should exist in MinIO bucket {string} with prefix {string} for room {string}")
  public void verifyRecordingFileExistsInMinIOWithPrefix(
      String bucketName, String prefix, String roomName) throws InterruptedException {
    verifyS3RecordingFile(bucketName, prefix, "recording-" + roomName, "room " + roomName);
  }

  @Then(
      "the track composite recording file should exist in MinIO bucket {string} for participant {string}")
  public void verifyTrackCompositeRecordingFileExistsInMinIO(
      String bucketName, String participantIdentity) throws InterruptedException {
  ...
  public void verifyTrackCompositeRecordingFileExistsInMinIOWithPrefix(
    ...
    verifyS3RecordingFile(
        bucketName,
        prefix,
        "track-composite-" + participantIdentity,
  ...
  @Then(
      "no track composite recording file should exist in the local output directory for participant {string}")
  ...
  private void createEgressContainer(
      String serviceName, String livekitServiceName, S3Config s3Config) {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    ...
    EgressContainer egressContainer =
        EgressContainer.createContainer(
            serviceName,
            containerManager.getOrCreateNetwork(),
  ...
  private void createEgressContainerWithS3(
      String serviceName, String livekitServiceName, S3Config s3Config) {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    ...
    EgressContainer egressContainer =
        EgressContainer.createContainerWithS3(
            serviceName,
            containerManager.getOrCreateNetwork(),
            TestConfig.getEgressVersion(),
            livekitWsUrl,
  ...
  private void startRoomCompositeRecordingToLocal(String roomName, String livekitServiceName)
      throws Exception {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    LiveKitContainer liveKitContainer =
        containerManager.getContainer(livekitServiceName, LiveKitContainer.class);
  ...
  private void startEgressRecording(
    ...
    try {
      egressInfo =
          egressClient.startRoomCompositeEgress(roomName, fileOutput, "grid").execute().body();
  ...
  private void stopEgressRecording(
      String egressId, String livekitServiceName, String stateKey, String description)
      throws Exception {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    LiveKitContainer liveKitContainer =
        containerManager.getContainer(livekitServiceName, LiveKitContainer.class);
    ...
    try {
      ...
      if (egressInfo == null) {
        ...
        egressInfo = LivekitEgress.EgressInfo.newBuilder().setEgressId(egressId).build();
      }
    } catch (Exception e) {
  ...
  private void verifyLocalRecordingFile(String filePattern, String description)
      throws InterruptedException {
    ...
    assertTrue(
        ...
        "Recording file does not exist: " + recordingFile.getAbsolutePath());
    assertTrue(
        recordingFile.length() > 0, "Recording file is empty: " + recordingFile.getAbsolutePath());

    log.info(
        "Recording file found: {} (size: {} bytes)",
        recordingFile.getName(),
        recordingFile.length());
  }

  private void verifyS3RecordingFile(
      String bucketName, String prefix, String filePattern, String description)
      throws InterruptedException {
    ContainerStateManager containerManager = ManagerProvider.getContainerManager();
    MinIOContainer minioContainer = containerManager.getContainer("minio", MinIOContainer.class);
    ...
    assertNotNull(
        recordingKey,
        "No recording files found in S3 bucket "
            + bucketName
            + " for "
            + description
            + " after 20 attempts");
  ...
  private void verifyLocalMultiParticipantContent() throws InterruptedException {
    ...
    log.info(
        "Verified multi-participant recording: {} ({} bytes)",
        recordingFile.getName(),
        recordingFile.length());
  }
  ...
  private void verifyS3VideoContent(String bucketName) {
    ...
    log.info(
        "Verified S3 recording contains actual video content: {} ({} bytes)", recordingKey, size);
  }
  ...
  private File pollForFile(File directory, String pattern, int maxAttempts)
      throws InterruptedException {
    int attempt = 0;
    ...
    while (attempt < maxAttempts) {
      ...
      if (attempt < maxAttempts - 1) {
        log.debug(
            "Recording file not ready yet, waiting... (attempt {}/{})", attempt + 1, maxAttempts);
        TimeUnit.MILLISECONDS.sleep(500);
      }
      attempt++;
    }
  ...
  private String pollForS3Object(
      MinioS3Client s3Client, String prefix, String pattern, int maxAttempts)
      throws InterruptedException {
    int attempt = 0;
    ...
    while (attempt < maxAttempts) {
      ...
      if (attempt < maxAttempts - 1) {
        log.debug(
            "Recording file not found in S3 yet, waiting... (attempt {}/{})",
            attempt + 1,
            maxAttempts);
