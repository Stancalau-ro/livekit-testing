...
public class LiveKitRoomSteps {
  ...
  public LivekitModels.Room createRoom(String serviceName, String roomName) {
    ...
    try {
      ...
      log.error(
          "Failed to create room '{}' using service '{}': {}",
          roomName,
          serviceName,
          e.getMessage(),
          e);
  ...
  public void theRoomCountShouldBe(int expectedCount) {
    ...
    assertEquals(
        expectedCount, lastFetchedRooms.size(), "Room count does not match expected value");
  }
  ...
  public List<LivekitModels.ParticipantInfo> getParticipantInfo(
      String serviceName, String roomName) {
  ...
  private LivekitModels.ParticipantInfo findParticipant(
      String serviceName, String roomName, String identity) {
  ...
  private LivekitModels.ParticipantInfo findParticipantOrFail(
      String serviceName, String roomName, String identity) {
    LivekitModels.ParticipantInfo participant = findParticipant(serviceName, roomName, identity);
    assertNotNull(
        participant, "Participant '" + identity + "' should exist in room '" + roomName + "'");
    return participant;
  }
  ...
  public void theRoomShouldHaveActiveParticipantsInService(
      String roomName, int expectedCount, String serviceName) {
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () -> getParticipantInfo(serviceName, roomName).size() == expectedCount);
    if (!success) {
      int actualCount = getParticipantInfo(serviceName, roomName).size();
      log.warn(
          "Participant count check failed. Expected: {}, Found: {}", expectedCount, actualCount);
      assertEquals(
          expectedCount,
          actualCount,
  ...
  public void participantShouldBePublishingVideoInRoomUsingService(
    ...
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () -> {
              LivekitModels.ParticipantInfo p =
                  findParticipantOrFail(serviceName, roomName, participantIdentity);
  ...
  @Then(
      "participant {string} should not be publishing video in room {string} using service {string}")
  public void participantShouldNotBePublishingVideoInRoomUsingService(
    ...
    assertEquals(
        0,
        videoTrackCount,
        "Participant '" + participantIdentity + "' should have 0 published video tracks");
  }
  ...
  public void roomIsDeletedUsingService(String roomName, String serviceName) {
    ...
    try {
      ...
      log.error(
          "Failed to delete room '{}' using service '{}': {}",
          roomName,
          serviceName,
          e.getMessage(),
          e);
  ...
  public void participantShouldAppearInRoomUsingService(
    ...
    assertTrue(
        success,
        "Participant '" + participantIdentity + "' should appear in room '" + roomName + "'");
  }

  @Then(
      "participant {string} should be publishing screen share in room {string} using service {string}")
        ...
        BrowserPollingHelper.pollForCondition(
            () -> {
              LivekitModels.ParticipantInfo p =
                  findParticipantOrFail(serviceName, roomName, participantIdentity);
              return p.getTracksList().stream()
                  .anyMatch(t -> t.getSource() == LivekitModels.TrackSource.SCREEN_SHARE);
            });
    if (!success) {
      LivekitModels.ParticipantInfo p = findParticipant(serviceName, roomName, participantIdentity);
      long screenShareCount =
          p != null
              ? p.getTracksList().stream()
  ...
  @Then(
      "participant {string} should not be publishing screen share in room {string} using service {string}")
  ...
  public void participantShouldHaveRemoteScreenShareTracksAvailableInRoomUsingService(
    ...
    long remoteScreenShareTrackCount =
        participants.stream()
            .filter(p -> !participantIdentity.equals(p.getIdentity()))
            .flatMap(p -> p.getTracksList().stream())
  ...
  @Then(
      "participant {string} should have {int} published tracks in room {string} using service {string}")
  public void participantShouldHavePublishedTracksInRoomUsingService(
    ...
    assertNotNull(
        targetParticipant,
        "Participant '" + participantIdentity + "' should exist in room '" + roomName + "'");
  ...
  private List<LivekitModels.VideoLayer> getVideoLayersForParticipant(
      String serviceName, String roomName, String participantIdentity) {
    LivekitModels.TrackInfo videoTrack =
        getVideoTrackForParticipant(serviceName, roomName, participantIdentity);
  ...
  @Then(
      "participant {string} should have simulcast {enabledState} for video in room {string} using service {string}")
  public void participantShouldHaveSimulcastState(
      String identity, EnabledState state, String room, String service) {
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () -> {
              LivekitModels.TrackInfo videoTrack =
                  getVideoTrackForParticipant(service, room, identity);
  ...
  @Then(
      "participant {string} video track should have {string} layers in room {string} using service {string}")
  public void videoTrackShouldHaveLayers(
      String identity, String layerExpression, String room, String service) {
    StringParsingUtils.ComparisonExpression comparison =
        StringParsingUtils.parseComparisonExpression(layerExpression);
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () ->
                comparison.evaluate(getVideoLayersForParticipant(service, room, identity).size()));
    if (success) {
      log.info(
          "Video layer check passed for participant '{}' (expression: {})",
          identity,
          layerExpression);
    } else {
  ...
  @Then(
      "participant {string} video layers should have different resolutions in room {string} using service {string}")
  public void videoLayersShouldHaveDifferentResolutions(
      String identity, String room, String service) {
    List<LivekitModels.VideoLayer> layers = getVideoLayersForParticipant(service, room, identity);
    assertTrue(
        layers.size() >= 2,
        "Need at least 2 layers to compare resolutions, found: " + layers.size());
  ...
  @Then(
      "participant {string} video layers should have different bitrates in room {string} using service {string}")
  public void videoLayersShouldHaveDifferentBitrates(String identity, String room, String service) {
    List<LivekitModels.VideoLayer> layers = getVideoLayersForParticipant(service, room, identity);
    assertTrue(
        layers.size() >= 2, "Need at least 2 layers to compare bitrates, found: " + layers.size());

    Set<Integer> bitrates =
        layers.stream().map(LivekitModels.VideoLayer::getBitrate).collect(Collectors.toSet());

    assertTrue(
        bitrates.size() > 1, "Video layers should have different bitrates, found: " + bitrates);
  }
  ...
  @Then(
      "the CLI publisher should have simulcast video layers in room {string} using service {string}")
  public void cliPublisherShouldHaveSimulcastLayers(String room, String service) {
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () ->
                getParticipantInfo(service, room).stream()
                    .flatMap(p -> p.getTracksList().stream())
                    .filter(t -> t.getType() == LivekitModels.TrackType.VIDEO)
  ...
  @Then(
      "the CLI publisher should have exactly {int} video layer in room {string} using service {string}")
  ...
  @Then(
      "participant {string} should have audio track {muteState} in room {string} using service {string}")
  public void participantShouldHaveAudioTrackStateInRoomUsingService(
    ...
    if (success) {
      log.info(
          "Participant '{}' has audio track {} in room '{}'", participantIdentity, state, roomName);
    } else {
      assertTrue(
          false, "Participant '" + participantIdentity + "' should have audio track " + state);
    }
  }

  @Then(
      "participant {string} should have video track {muteState} in room {string} using service {string}")
  public void participantShouldHaveVideoTrackStateInRoomUsingService(
    ...
    if (success) {
      log.info(
          "Participant '{}' has video track {} in room '{}'", participantIdentity, state, roomName);
    } else {
      assertTrue(
          false, "Participant '" + participantIdentity + "' should have video track " + state);
    }
  }
}
