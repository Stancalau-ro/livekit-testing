...
public class LiveKitImageSnapshotSteps {
  ...
  public void captureRoomSnapshotToS3(
      String roomName, String livekitServiceName, String minioServiceName) throws Exception {
  ...
  public void verifyRoomSnapshotExistsInS3(String roomName, String minioServiceName)
      throws Exception {
  ...
  public void verifyParticipantSnapshotExistsInS3(
      String participantIdentity, String minioServiceName) throws Exception {
  ...
  private void captureRoomSnapshot(
    ...
    log.info(
        "Room {} has {} participants with video tracks: {}",
        roomName,
        participants.size(),
        hasVideoTrack);
    ...
    try {
      retrofit2.Response<LivekitEgress.EgressInfo> response =
          egressClient.startRoomCompositeEgress(roomName, imageOutput, layout).execute();
      log.info(
          "Snapshot egress API HTTP response code: {}, message: {}",
          response.code(),
          response.message());
    ...
    ManagerProvider.getImageSnapshotStateManager()
        .storeCapturedSnapshot(stateKey, fileName + ".jpg");
  ...
  private void verifyS3SnapshotExists(String stateKey, String errorMessage, String minioServiceName)
    ...
    String expectedFileName =
        ManagerProvider.getImageSnapshotStateManager().getCapturedSnapshot(stateKey);
    ...
    while (attempt < maxAttempts && downloadedFile == null) {
      log.debug(
          "Checking for snapshot files in S3 bucket... (attempt {}/{})", attempt + 1, maxAttempts);
    ...
    assertNotNull(
        downloadedFile,
        "No snapshot image files found in S3 bucket for prefix: " + expectedFilePrefix);
    assertTrue(
        downloadedFile.exists(),
        "Downloaded snapshot file does not exist: " + downloadedFile.getAbsolutePath());
    assertTrue(
        downloadedFile.length() > 0,
        "Downloaded snapshot file is empty: " + downloadedFile.getAbsolutePath());
  ...
  public void aLiveKitS3SnapshotEgressServiceIsRunningInContainer(
      String serviceName, String livekitServiceName) {
    createEgressContainer(
        serviceName, livekitServiceName, EgressContainer::createContainer, "S3 snapshot");
  }

  @Given(
      "a LiveKit snapshot egress service is running in a container with service name {string} connected to LiveKit service {string}")
  public void aLiveKitSnapshotEgressServiceIsRunningInContainer(
      String serviceName, String livekitServiceName) {
    createEgressContainer(
        serviceName, livekitServiceName, EgressContainer::createContainer, "Snapshot");
  }

  @When(
      "an on-demand snapshot is captured to local filesystem for room {string} using LiveKit service {string}")
  public void captureRoomSnapshotToLocalFilesystem(String roomName, String livekitServiceName)
      throws Exception {
    String timestamp = DateUtils.generateRecordingTimestamp();
  ...
  public void verifyLocalRoomSnapshotExists(String roomName) throws Exception {
    ...
    while (attempt < maxAttempts && bestFile == null) {
      ...
      File[] snapshotFiles =
          snapshotsDir.listFiles(
              (dir, name) ->
                  name.startsWith(
                          expectedFilePrefix.substring(expectedFilePrefix.lastIndexOf("/") + 1))
    ...
    assertNotNull(
        bestFile, "No local snapshot image files found for prefix: " + expectedFilePrefix);
    assertTrue(
        bestFile.exists(), "Local snapshot file does not exist: " + bestFile.getAbsolutePath());
    assertTrue(
        bestFile.length() > 0, "Local snapshot file is empty: " + bestFile.getAbsolutePath());

    log.info(
        "Local room snapshot file found: {} (size: {} bytes)",
  ...
  public void verifyLocalParticipantSnapshotExists(String participantIdentity) throws Exception {
    ...
    assertNotNull(
        expectedFileName,
        "No local track snapshot recorded for participant " + participantIdentity);
    ...
    while (attempt < maxAttempts && bestFile == null) {
      log.debug(
          "Checking for local participant track snapshot files... (attempt {}/{})",
          attempt + 1,
          maxAttempts);
    ...
    assertNotNull(
        bestFile,
        "No local participant track snapshot image files found for prefix: " + expectedFilePrefix);
    assertTrue(
        bestFile.exists(),
        "Local participant track snapshot file does not exist: " + bestFile.getAbsolutePath());
    assertTrue(
        bestFile.length() > 0,
        "Local participant track snapshot file is empty: " + bestFile.getAbsolutePath());
  ...
  private void cleanupBlackFramesFromS3(
      String bucketName, String filePrefix, String minioServiceName) {
    try {
      ContainerStateManager containerManager = ManagerProvider.getContainerManager();
      MinIOContainer minioContainer =
          containerManager.getContainer(minioServiceName, MinIOContainer.class);
      ...
      for (String objectToDelete : filesToDelete) {
        ...
        log.info(
            "Deleting smaller/duplicate frame from S3: {} (size: {} bytes)", objectToDelete, size);
  ...
  private void validateSnapshotImage(String prefix, String extensions, String type)
      throws Exception {
    File snapshotsDir = new File(getCurrentScenarioLogPath(), "snapshots");
    assertTrue(
    ...
    assertNotNull(
        ...
        "No "
            + type.toLowerCase()
            + " image files found in directory: "
    ...
    assertTrue(
        ...
        type
            + " image file too small (< 1KB), likely corrupted or empty: "
  ...
  private void createEgressContainer(
      String serviceName,
      String livekitServiceName,
      EgressContainerFactory factory,
    ...
    assertTrue(
        ...
        containerType
            + " egress container with service name "
            + serviceName
            + " should be running");
