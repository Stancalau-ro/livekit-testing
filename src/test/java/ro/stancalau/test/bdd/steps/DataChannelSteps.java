...
public class DataChannelSteps {
  ...
  public void shouldReceiveDataMessageFrom(String receiver, String message, String sender) {
    ...
    assertTrue(
        received, receiver + " should have received data message '" + message + "' from " + sender);
  }

  @Then("{string} should receive at least {int} out of {int} messages from {string}")
  public void shouldReceiveAtLeastMessagesFrom(
      String receiver, int minMessages, int totalSent, String sender) {
    boolean receivedEnough =
        ManagerProvider.dataChannel()
  ...
  public void shouldReceiveAllTimestampedMessages(String participantName) {
    int receivedCount =
        ManagerProvider.dataChannel().getReceivedMessageCountAfterWait(participantName);
    assertTrue(
        receivedCount >= MIN_TIMESTAMPED_MESSAGES,
        participantName
            + " should have received all timestamped messages (received: "
            + receivedCount
            + ")");
  }
  ...
  public void shouldNotReceiveDataMessage(String participantName, String message) {
    boolean received =
        ManagerProvider.dataChannel().hasReceivedMessageAfterWait(participantName, message);
  ...
  public void shouldHaveDataPublishingBlockedDueToPermissions(String participantName) {
    ...
    assertTrue(
        isBlocked, participantName + " should have data publishing blocked (error: " + error + ")");
  }
  ...
  @Then(
      "the test logs document that lossy mode in local containers typically achieves near-100% delivery")
  public void testLogsDocumentLossyModeDelivery() {
    log.info(
        "NOTE: Lossy/unreliable data channel mode in local container testing typically "
  ...
  public void shouldReceiveDataMessagesInOrder(String participantName, DataTable dataTable) {
    ...
    boolean allReceived =
        ManagerProvider.dataChannel()
            .waitForMessageCount(
                ...
                LiveKitMeet.BATCH_DATA_MESSAGE_TIMEOUT_MS);
    assertTrue(
        allReceived,
    ...
    log.info(
        "All {} messages received in correct order for {}",
        expectedMessages.size(),
        participantName);
  }
}
