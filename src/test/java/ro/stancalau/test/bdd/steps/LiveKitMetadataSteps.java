...
public class LiveKitMetadataSteps {
  ...
  public void verifyRoomMetadata(String roomName, String expectedMetadata, String serviceName) {
    boolean success =
        ManagerProvider.metadata()
            .pollUntilRoomMetadataEquals(serviceName, roomName, expectedMetadata);
  ...
  public void verifyRoomMetadataLength(String roomName, int expectedLength, String serviceName) {
    ...
    assertEquals(
        expectedLength, metadata.length(), "Room metadata length should be " + expectedLength);
  }
  ...
  public void verifyRetrievedRoomInfoEmptyMetadata() {
    ...
    assertTrue(
        metadata == null || metadata.isEmpty(), "Retrieved room info should have empty metadata");
  }

  @When(
      "participant {string} metadata is updated to {string} in room {string} using service {string}")
  public void updateParticipantMetadata(
      String identity, String metadata, String roomName, String serviceName) {
  ...
  @Then(
      "participant {string} should have metadata {string} in room {string} using service {string}")
  public void verifyParticipantMetadata(
      String identity, String expectedMetadata, String roomName, String serviceName) {
    boolean success =
        ManagerProvider.metadata()
            .pollUntilParticipantMetadataEquals(serviceName, roomName, identity, expectedMetadata);
  ...
  @Then(
      "participant {string} should have metadata containing {string} in room {string} using service {string}")
  public void verifyParticipantMetadataContains(
      String identity, String expectedSubstring, String roomName, String serviceName) {
    String metadata =
        BrowserPollingHelper.pollUntil(
            () ->
                ManagerProvider.metadata().getParticipantMetadata(serviceName, roomName, identity),
    ...
    assertTrue(
        metadata != null && metadata.contains(expectedSubstring),
  ...
  public void verifyRoomMetadataEvent(String participantName, String expectedValue) {
    boolean received =
        ManagerProvider.metadata().waitForRoomMetadataEvent(participantName, expectedValue);
    if (!received) {
      List<Map<String, Object>> events =
          ManagerProvider.metadata().getRoomMetadataEvents(participantName);
      log.warn(
          "{} did not receive room metadata event with value '{}'. Received events: {}",
          participantName,
          expectedValue,
          events);
    }
    assertTrue(
        received,
        participantName + " should receive room metadata event with value: " + expectedValue);
  }

  @Then(
      "{string} should receive a participant metadata update event for {string} with value {string}")
  public void verifyParticipantMetadataEvent(
      String participantName, String targetIdentity, String expectedValue) {
    boolean received =
        ManagerProvider.metadata()
            .waitForParticipantMetadataEvent(participantName, targetIdentity, expectedValue);
    assertTrue(
        received,
        participantName
            + " should receive participant metadata event for "
            + targetIdentity
            + " with value: "
            + expectedValue);
  }
  ...
  public void verifyParticipantMetadataEventCount(String participantName, int expectedCount) {
    ...
    assertTrue(
        ...
        participantName
            + " should have received at least "
            + expectedCount
            + " participant metadata events but got: "
            + actualCount);
  }
  ...
  public void verifyBrowserParticipantMetadata(
      String observerName, String targetIdentity, String expectedMetadata) {
    String actual =
        BrowserPollingHelper.pollUntil(
            () ->
                ManagerProvider.metadata()
                    .getParticipantMetadataFromBrowser(observerName, targetIdentity),
            expectedMetadata::equals);
    assertEquals(
        expectedMetadata,
        actual,
        observerName
            + " should see participant "
            + targetIdentity
            + " with metadata: "
            + expectedMetadata);
  }
  ...
  public void verifyBrowserParticipantMetadataContains(
      String observerName, String targetIdentity, String expectedSubstring) {
    String actual =
        BrowserPollingHelper.pollUntil(
            () ->
                ManagerProvider.metadata()
  ...
  @Then(
      "participant {string} metadata should be valid JSON in room {string} using service {string}")
  public void verifyParticipantMetadataIsValidJson(
      String identity, String roomName, String serviceName) {
    String metadata =
        ManagerProvider.metadata().getParticipantMetadata(serviceName, roomName, identity);
  ...
  @Then(
      "participant {string} should have metadata of approximately {int} bytes in room {string} using service {string}")
  public void verifyParticipantMetadataApproximateSize(
      String identity, int expectedBytes, String roomName, String serviceName) {
    String metadata =
        ManagerProvider.metadata().getParticipantMetadata(serviceName, roomName, identity);
    assertNotNull(metadata, "Metadata should not be null");
    int tolerance = expectedBytes / 10;
    assertTrue(
        Math.abs(metadata.length() - expectedBytes) <= tolerance,
        "Metadata length should be approximately "
            + expectedBytes
            + " bytes (got "
            + metadata.length()
            + ")");
