...
public class LiveKitAccessTokenSteps {
    ...
    @When("the system creates an access token with identity {string} and room {string} with grants {string} and metadata {string}")
    @Given("an access token is created with identity {string} and room {string} with grants {string} and metadata {string}")
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndMetadata(String identity, String roomName, String grantsString, String metadata) {
        List<String> grants = StringParsingUtils.parseCommaSeparatedList(grantsString);
        AccessToken token = ManagerProvider.tokens().createTokenWithDynamicGrants(identity, roomName, grants, null, metadata);
        assertNotNull(token, "Access token should be created");
    }

    @When("the system creates an access token with identity {string} and room {string} with grants {string} and metadata of size {int} bytes")
    @Given("an access token is created with identity {string} and room {string} with grants {string} and metadata of size {int} bytes")
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndMetadataOfSize(String identity, String roomName, String grantsString, int sizeBytes) {
        List<String> grants = StringParsingUtils.parseCommaSeparatedList(grantsString);
    ...
    public void theAccessTokenForInRoomShouldHaveMetadataOfLength(String identity, String roomName, int expectedLength) {
        AccessToken token = ManagerProvider.tokens().getLastToken(identity, roomName);
        assertNotNull(token, "Access token for " + identity + " in room " + roomName + " should not be null");
