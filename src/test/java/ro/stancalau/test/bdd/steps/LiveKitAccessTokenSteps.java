...
public class LiveKitAccessTokenSteps {
    ...
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndAttributes(
        ...
        AccessToken token =
                ManagerProvider.tokens().createTokenWithDynamicGrants(identity, roomName, grants, attributes);
        assertNotNull(token, "Access token should be created");
    }
    ...
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsThatExpiresInSeconds(
        ...
        AccessToken token =
                ManagerProvider.tokens().createTokenWithDynamicGrants(identity, roomName, grants, null, ttlMillis);
        assertNotNull(token, "Access token should be created");
    }
    ...
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndAttributesThatExpiresInSeconds(
            String identity, String roomName, String grantsString, String attributesString, int seconds) {
        List<String> grants = StringParsingUtils.parseCommaSeparatedList(grantsString);
        Map<String, String> attributes = StringParsingUtils.parseKeyValuePairs(attributesString);
        long ttlMillis = seconds * 1000L;
        AccessToken token = ManagerProvider.tokens()
    ...
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndAttributesThatExpiresInMinutes(
            String identity, String roomName, String grantsString, String attributesString, int minutes) {
        List<String> grants = StringParsingUtils.parseCommaSeparatedList(grantsString);
        Map<String, String> attributes = StringParsingUtils.parseKeyValuePairs(attributesString);
        long ttlMillis = minutes * 60 * 1000L;
        AccessToken token = ManagerProvider.tokens()
    ...
    public void theAccessTokenForInRoomShouldHaveAttributeSetTo(
        ...
        assertEquals(expectedValue, actualValue, "Attribute " + attributeName + " should be set to " + expectedValue);
    ...
    public void theAccessTokenForInRoomShouldNotHaveAttribute(String identity, String roomName, String attributeName) {
        ...
        Map<String, Object> attributes = getTokenAttributes(decodedJWT);
        assertFalse(attributes.containsKey(attributeName), "Token should not contain attribute " + attributeName);
    ...
    private String normalizeExpectedValue(String expected) {
        if (expected != null && expected.startsWith("[") && expected.endsWith("]")) {
            ...
            List<String> items =
                    Arrays.stream(inner.split(",")).map(String::trim).toList();
    ...
    public void theAccessTokenForInRoomShouldHaveTheFollowingAttributes(
        ...
        for (Map<String, String> attributeRow : attributeRows) {
            ...
            String actualValue = attributes.get(attributeName).toString();
            assertEquals(
                    expectedValue, actualValue, "Attribute " + attributeName + " should be set to " + expectedValue);
    ...
    public void theSystemCreatesAccessTokenWithIdentityAndRoomWithGrantsAndMetadata(
        ...
        List<String> grants = StringParsingUtils.parseCommaSeparatedList(grantsString);
        AccessToken token =
                ManagerProvider.tokens().createTokenWithDynamicGrants(identity, roomName, grants, null, metadata);
        assertNotNull(token, "Access token should be created");
    }
    ...
    public void theAccessTokenForInRoomShouldHaveMetadata(String identity, String roomName, String expectedMetadata) {
        AccessToken token = ManagerProvider.tokens().getLastToken(identity, roomName);
        assertNotNull(token, "Access token for " + identity + " in room " + roomName + " should not be null");
        ...
        log.info("Verified metadata is set to {} for {} in room {}", expectedMetadata, identity, roomName);
    }
    ...
    public void theAccessTokenForInRoomShouldHaveMetadataOfLength(
        ...
        assertEquals(expectedLength, actualMetadata.length(), "Token metadata length should match expected value");

        log.info("Verified metadata length is {} for {} in room {}", expectedLength, identity, roomName);
    }
}
