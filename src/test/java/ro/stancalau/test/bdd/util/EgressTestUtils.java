...
public class EgressTestUtils {
  ...
  public static void waitForEgressToBeActive(
      EgressServiceClient egressClient, String egressId, String roomName) throws Exception {
    ...
    boolean success =
        BrowserPollingHelper.pollForCondition(
            () -> {
              try {
                LivekitEgress.EgressInfo info =
                    egressClient.listEgress(roomName).execute().body().stream()
