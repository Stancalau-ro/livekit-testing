...
public class EgressTestUtils {
    ...
    public static void waitForEgressToBeActive(EgressServiceClient egressClient, String egressId, String roomName) throws Exception {
            ...
            try {
                ...
                if (info != null) {
                    ...
                    if (status == LivekitEgress.EgressStatus.EGRESS_ACTIVE) {
                        ...
                        errorRef.set("Egress " + egressId + " failed to start. Status: " + status + ". Error: " + info.getError());
                        return true;
                    }
                }
                return false;
        ...
        if (!success) {
            throw new IllegalStateException("Egress " + egressId + " did not become active after " + MAX_ATTEMPTS + " seconds");
        }
    }
}