Feature: LiveKit MinIO Recording
  ...
  Scenario: Record video from a publisher to MinIO S3 bucket using egress
    Given the system creates room "S3RecordingRoom" using service "livekit1"
    ...
    When the system starts room composite recording for room "S3RecordingRoom" using LiveKit service "livekit1" with S3 output to bucket "recordings"
    And the recording runs for 6 seconds
    And the system stops room composite recording for room "S3RecordingRoom" using LiveKit service "livekit1"
  ...
  Scenario: Record specific participant tracks to MinIO S3 using track composite egress
    Given the system creates room "S3TrackCompositeRoom" using service "livekit1"
    ...
    And the system starts track composite recording for participant "James" in room "S3TrackCompositeRoom" using LiveKit service "livekit1" with S3 output to bucket "recordings"
    And the recording runs for 6 seconds
    And the system stops track composite recording for participant "James" using LiveKit service "livekit1"
  ...
  Scenario: Record multiple participants to separate S3 prefixes
    Given the system creates room "S3MultiPrefixRoom" using service "livekit1"
    ...
    And the system starts track composite recording for participant "Alice" in room "S3MultiPrefixRoom" using LiveKit service "livekit1" with S3 output to bucket "recordings" with prefix "alice/"
    And the system starts track composite recording for participant "Bob" in room "S3MultiPrefixRoom" using LiveKit service "livekit1" with S3 output to bucket "recordings" with prefix "bob/"
    And the recording runs for 6 seconds
