Feature: LiveKit Image Snapshots
  ...
  Scenario: Capture on-demand room snapshots to both S3 and local filesystem
    Given a MinIO server is running in a container with service name "minio"
    And a bucket "snapshots" is created in MinIO service "minio"
    And a LiveKit S3 snapshot egress service is running in a container with service name "egress" connected to LiveKit service "livekit"
    And room "SnapshotRoom" is created using service "livekit"
    ...
    Then participant "Alice" should be publishing video in room "SnapshotRoom" using service "livekit"
    And participant "Charlie" should be publishing video in room "SnapshotRoom" using service "livekit"
    
    When an on-demand snapshot is captured to S3 for room "SnapshotRoom" using LiveKit service "livekit" and MinIO service "minio"
    And an on-demand snapshot is captured to local filesystem for room "SnapshotRoom" using LiveKit service "livekit"
    
    Then the S3 snapshot image file exists for room "SnapshotRoom" using MinIO service "minio"
  ...
  Scenario: Capture on-demand participant track snapshots to both S3 and local filesystem
    ...
    And a bucket "snapshots" is created in MinIO service "minio"
    And a LiveKit S3 snapshot egress service is running in a container with service name "egress" connected to LiveKit service "livekit"
    And room "ParticipantSnapshotRoom" is created using service "livekit"
    ...
    Then participant "Bob" should be publishing video in room "ParticipantSnapshotRoom" using service "livekit"
    
    When an on-demand snapshot is captured to S3 for participant "Bob" video track in room "ParticipantSnapshotRoom" using LiveKit service "livekit" and MinIO service "minio"
    And an on-demand snapshot is captured to local filesystem for participant "Bob" video track in room "ParticipantSnapshotRoom" using LiveKit service "livekit"
    
    Then the S3 snapshot image file exists for participant "Bob" using MinIO service "minio"
    And the S3 snapshot image is valid and contains actual image data
